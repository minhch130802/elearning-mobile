...
import '/resources/api/util/util.dart';
...
class EnrolRestAPI extends EnrolUtil {
  ...
  Future<bool> selfEnrollment(int courseId, String password) async {
    FormData formData = new FormData.fromMap(await super.selfEnrolUser(courseId, password));
  ...
  Future<EnrolModel> getCourseEnrolMethod(int courseId) async {
    FormData formData = new FormData.fromMap(await super.getEnrolMethod(courseId));
  ...
  Future<EnrolModel> getCourseEnrolMethodInfo(EnrolModel model) async {
    FormData formData = new FormData.fromMap(await super.getEnrolMetodInfo(model.id!));
